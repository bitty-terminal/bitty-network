...
//! can narrow it but never widen it. WebSocket message budgets are
//! follow-up scope.
//!
...
impl HttpNetworkService {
    ...
    ///
    /// Redirect following is disabled (`Policy::none`): every hop is
    /// canonicalized and re-authorized in [`send`](Self::send) instead, so
    /// the capability sees each destination before it is contacted. The
    /// builder only fails on contradictory configuration, which the
...
///
/// Redirect following is disabled here exactly as in
