...
Step 2, and the destination-origin stripping in step 5, are implemented on the
base pin. Every other clause of steps 1 to 5 — step 1, step 3, step 4, and
step 5's proxy-origin half — is **[specified, not implemented]**, as is every
credential-resolving clause in the list. Two of those clauses must not be
credited even in part:

- Step 1 requires two things of the same clause, and the base pin does only
  the first, in a form this record refuses to count. Resolving the location
  against the previous effective URL is **[specified, not implemented]** here,
  because the clause also requires canonicalizing the new destination and no
  code on the base pin canonicalizes it. What the base pin runs is
  `resolve_redirect` (`http.rs:636`-`:666`) — the best-effort string surgery
  that [Canonical origins](#canonical-origins) explicitly declines to credit
  with canonicalization — and the origin comparison it feeds, `same_origin`
  (`http.rs:689`-`:697`), is built from `Request::host()` and `Request::port()`.
  Resolving a relative `Location` against the previous effective URL is the
  only part of step 1 that exists, and existing is not the clause.
- Step 4 is **[specified, not implemented]** because it is vacuous rather than
  absent. It requires the removal of the prior proxy authorization and every
  hop-specific connection or tunnel, and on the base pin no proxy
  authorization, no hop-specific connection, and no tunnel exists, so there is
  nothing to remove. No code performs the removal, and an empty removal is not
  a performed one.

The destination-origin stripping credited above is the whole of the implemented
part of the list. It strips caller headers, and it resolves nothing,
authorizes nothing, and dials nothing new.
...
(`:307`-`:321`, `:355`-`:361`, called from `:774` and `:785`); `proxy_client`
...
Those two forms are the whole taxonomy; no third qualifier is defined or used.
A requirement that is a list of clauses whose states differ — the redirect
step list is the only one — carries both forms in its Disposition cell, each
attached to the clauses it applies to, because collapsing such a requirement to
either form alone would state something false about code.
...
| --------------------------------------------------------------- | -------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B1 redirect ownership                                           | partial  | "Redirects, proxy responses, and `CONNECT`", steps 1-5                 | closed as a requirement; **[specified, not implemented]** for step 1, step 3, step 4, and step 5's proxy-origin half; implemented on the base pin, unmerged, for step 2 and step 5's destination-origin stripping |
...
whether a consumer can obtain that code today.

The State column carries one word that decides everything else. `present` means
the control exists on the base pin; a cell may add a qualifier after `present`
that narrows _what_ is present and never _whether_ it is, as in "present, but
wired into no caller". `not present` means the control exists on neither the
base pin nor `de77e17`, and is therefore a requirement of this record rather
than a claim about existing code. Those are the only two states.

Location cells name symbols, not line numbers. A citation is written
`` `path::anchor` ``, where `path` is a full repository-relative path and
`anchor` is text that must occur verbatim in that file; a negative claim is
written `` `path::[absent] anchor` ``, where `anchor` must occur nowhere in
that file. The pseudo-path `tree` stands for every Rust source file under
`crates/`, and is only valid with `[absent]`.

Line numbers were the previous form and they were the defect this record's
later review rounds kept finding: a rename or an insertion silently invalidates
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Hand-written redacting `Debug` for `HttpNetworkService`                                                                                                 | present                           | unmerged, PR #44 | `crates/bitty-network/src/http.rs::pub struct HttpNetworkService {`; `crates/bitty-network/src/http.rs::impl std::fmt::Debug for HttpNetworkService {`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `Egress` has neither a `Debug` derive nor a `Debug` impl                                                                                                | present                           | unmerged, PR #44 | `crates/bitty-network/src/http.rs::struct Egress {`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `Egress` no longer retains an `https_proxy` field                                                                                                       | present                           | unmerged, PR #44 | `crates/bitty-network/src/http.rs::struct ProxyRoute {`; `crates/bitty-network/src/http.rs::struct Egress {`; `crates/bitty-network/src/http.rs::[absent] https_proxy:`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Named regression `authenticated_proxy_is_rejected_before_dial`                                                                                          | present                           | unmerged, PR #44 | `crates/bitty-network/src/websocket.rs::fn authenticated_proxy_is_rejected_before_dial() {`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `.no_proxy()` and `Policy::none()` on every reqwest client construction                                                                                 | present                           | unmerged, PR #44 | `crates/bitty-network/src/http.rs::fn client_with(proxy: Option<reqwest::Proxy>) -> Option<reqwest::blocking::Client> {`; `crates/bitty-network/src/http.rs::fn proxy_client(url: &str) -> Option<reqwest::blocking::Client> {`; `crates/bitty-network/src/http.rs::fn proxy_route_client(route: &ProxyRoute) -> Option<reqwest::blocking::Client> {`; `crates/bitty-network/tests/http.rs::fn every_client_builder_disables_ambient_proxy_and_redirects() {`                                                                                                                                                                                                                                                                                                                                     |
| Redirect following disabled on every client; owned bounded hop loop                                                                                     | present                           | unmerged, PR #44 | `crates/bitty-network/src/http.rs::pub const MAX_REDIRECT_HOPS: usize = 5;`; `crates/bitty-network/src/http.rs::fn send(&self, request: &Request) -> Result<Response, NetworkError> {`; `crates/bitty-network/src/http.rs::fn client_with(proxy: Option<reqwest::Proxy>) -> Option<reqwest::blocking::Client> {`; `crates/bitty-network/src/http.rs::fn proxy_client(url: &str) -> Option<reqwest::blocking::Client> {`                                                                                                                                                                                                                                                                                                                                                                           |
| Fixed `CONNECT` target; no target change after the tunnel opens                                                                                         | present                           | unmerged, PR #44 | `crates/bitty-network/src/websocket.rs::fn tunnel_via_proxy(`; `crates/bitty-network/src/websocket.rs::fn format_authority(`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Sensitive-header stripping on a destination-origin change                                                                                               | present                           | unmerged, PR #44 | `crates/bitty-network/src/http.rs::const STRIPPED_CROSS_ORIGIN_HEADERS`; `crates/bitty-network/src/http.rs::fn send(&self, request: &Request) -> Result<Response, NetworkError> {`; `crates/bitty-network/src/http.rs::fn same_origin(first: &str, second: &str) -> bool {`; `crates/bitty-network/src/http.rs::fn strip_cross_origin_headers(`; `crates/bitty-network/tests/http.rs::fn cross_origin_redirect_strips_sensitive_headers() {`                                                                                                                                                                                                                                                                                                                                                      |
| Redacted diagnostic snapshot constructors                                                                                                               | present, but wired into no caller | unmerged, PR #44 | `crates/bitty-network/src/diagnostics.rs::pub fn redacted_url(url: &str) -> String {`; `crates/bitty-network/src/diagnostics.rs::pub fn redacted_headers(`; `crates/bitty-network/src/diagnostics.rs::pub fn summarize_body(`; `crates/bitty-network/src/diagnostics.rs::pub fn connect_authority(`; `crates/bitty-network/src/diagnostics.rs::pub struct DiagnosticRequest {`; `crates/bitty-network/src/diagnostics.rs::is a follow-up merge`; `crates/bitty-network/src/http.rs::[absent] redacted_url`; `crates/bitty-network/src/websocket.rs::[absent] redacted_url`. The two `[absent]` anchors are what mechanically proves the unwired claim for both backends; that no module anywhere in the crate calls these constructors is a reading of the rest of the tree and is held by review |
| Named regression `credentialed_proxy_fails_closed_without_debug_secret`                                                                                 | not present                       | not applicable   | `tree::[absent] credentialed_proxy_fails_closed_without_debug_secret`; `crates/bitty-network/src/http.rs::fn explicit_credentialed_proxy_is_rejected_without_exposed_secret() {`. The property it was meant to guard — a credential cannot escape through the service's `Debug` — is instead pinned by `http_network_service_debug_is_hand_written_and_cannot_emit_a_credential`; the citation above is the nearest pre-existing coverage of that property                                                                                                                                                                                                                                                                                                                                        |
| Sensitive-header stripping on a **proxy-origin-only** change (N1)                                                                                       | not present                       | not applicable   | `crates/bitty-network/src/http.rs::fn send(&self, request: &Request) -> Result<Response, NetworkError> {`; `crates/bitty-network/src/http.rs::fn same_origin(first: &str, second: &str) -> bool {`. The stripping condition inside `send` is `!same_origin(&url, &destination)`, a destination comparison with no proxy term; the absence of a proxy term is a reading of that expression and is held by review, not by this citation check. The requirement is marked **[specified, not implemented]** in [Redirects, proxy responses, and `CONNECT`](#redirects-proxy-responses-and-connect)                                                                                                                                                                                                    |
...
| `Request`, `WebSocketRequest`, and `Response` free of derived `Debug` and `PartialEq`                                                                   | not present                       | not applicable   | `crates/bitty-network-api/src/lib.rs::pub struct Request {`; `crates/bitty-network-api/src/lib.rs::pub struct WebSocketRequest {`; `crates/bitty-network-api/src/lib.rs::pub struct Response {`. **[specified, not implemented]** and pinned in both directions by `api_vocabulary_types_still_derive_debug_and_equality_and_still_leak`, so the derive set cannot change without the change failing                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `ProxyCredentialProvider`, `CanonicalOrigin`, scope registry, pool key, `AuthorizationLease`, `proxy::inject_authorization`, remote-revocation evidence | not present                       | not applicable   | `tree::[absent] ProxyCredentialProvider`; `tree::[absent] CanonicalOrigin`; `tree::[absent] AuthorizationLease`; `tree::[absent] inject_authorization`; `crates/bitty-network/src/proxy.rs::pub fn env_proxy_enabled() -> bool {`. The scope registry, the pool key, and the revocation evidence have no identifier to check, so they are not cited here; `crates/bitty-network/src/proxy.rs` is a feature-gate predicate module and nothing else. This record defines them                                                                                                                                                                                                                                                                                                                       |
...
| Child-process failed-`PartialEq`-assertion redaction coverage                                                                                           | not present                       | not applicable   | `crates/bitty-network/tests/http.rs::fn run_proxy_child_process(`; `crates/bitty-network/tests/http.rs::fn ambient_proxy_environment_is_explicit_and_credential_safe() {`; `crates/bitty-network-api/src/lib.rs::pub struct Request {`. The child process and the credential cases that scan its combined stdout and stderr exist, but nothing on the base pin triggers a failed equality assertion on a credential-bearing type, because the only credential-bearing types with a derived `Debug` are the API vocabulary types, which no backend formats                                                                                                                                                                                                                                         |
| Environment-proxy inheritance gated on the `proxy` feature                                                                                              | not present                       | not applicable   | `crates/bitty-network/src/http.rs::pub fn new(capability: NetworkCapability) -> Self {`; `crates/bitty-network/src/proxy.rs::pub fn env_proxy_enabled() -> bool {`; `crates/bitty-network/src/http.rs::[absent] env_proxy_enabled`. `HttpNetworkService::new` calls `ProxyRoute::from_env` unconditionally and `http.rs` never calls the gate. Owned by CTX-0034 and remediated under CTX-0043; recorded here as a fact about the base pin, not as this record's scope                                                                                                                                                                                                                                                                                                                            |
...
The citation pin was checked six ways, and each one was observed to fail while
the rest of the suite stayed green: a consistent rename of a cited symbol with
this record left stale, a symbol name in a Location cell that does not exist, a
forbidden identifier actually appearing in the tree so an `[absent]` claim goes
stale, an abbreviated path, a mistyped path, and the `tree` pseudo-path used as
a presence claim. It also caught two real defects on its first run against this
record: a bare `tree` locator carrying no anchor, and an `[absent] https_proxy`
...
check of those would be unsound — a count is satisfied by one marker anywhere
and a context window by any marker near any heading, so it would go green while
the defect recurred, and a false green on a security guarantee is worse than no
check — and it would reward adding markers to implemented requirements, turning
a judgement into a checkbox. The markers stay review-enforced, which is the
honest position.
